...
### 15.4 双语开发日志 `DEVLOG.md`

项目根目录必须维护 `DEVLOG.md`，**英文与中文双语**，记录实际发生的开发过程：做出的决定、实测得到的事实，以及被推翻的原有假设。

与其他文档的分工：

| 文档 | 职责 |
|---|---|
| `CHANGELOG.md` | 对使用者可见的变化，简短陈述 |
| `DEVLOG.md` | 为什么这么做、测了什么、还有什么没关闭 |
| `docs/decisions/` | 单个重大决策的完整论证 |
| `docs/resonance-audit.md` | 一次性的原型审计结论 |

硬性要求：

- 单一 H1，其后分为 `## English` 与 `## 中文` 两部分，条目内容与顺序一致。
- 每个条目是一个 `### ` 标题，包含日期与所属 Phase。
- 两部分必须在同一个提交内一起更新。
- `tools/mdcheck.nim` 校验 `DEVLOG.md` 存在、两个语言部分都存在、且两部分条目数相等。条目数不等即 `nimble lint` 失败。

按语言分成两半而不是逐条交替，是为了让 §6.7.9 要求的"标题名称唯一"成立——逐条交替会产生大量重复的 `### English` 标题。代价是两半容易漂移，因此用条目数校验兜底。该校验无法证明两半说的是同一件事，但能抓住"只加了一边"这个最常见的失误。
...
| Gate 条目 | 状态 | 证据 |
|---|---|---|
| Tier 1 阻断 CI 全绿 | 通过 | 重命名后的公开仓库 `AbyssGG/OpenVINO-Nim-API` 在 commit `de49201` 的 [CI #36056011323](https://github.com/AbyssGG/OpenVINO-Nim-API/actions/runs/36056011323) 成功；static、双平台 ABI/smoke/lifecycle/integration/examples-package、Nim 2.0.0/2.2.12 ORC/ARC 单元测试与 docs 全部通过。定时 memory 作业按设计不在 push 运行，其独立 valgrind 证据见 F12 |
...
- [x] C12 集中实现 Windows/Linux 动态库名和编译期覆盖。 证据：`private/library.nim` 集中平台库名并提供 `-d:openvinoLib=` 编译期覆盖。**Linux 首次实测发现并修复了一个真实缺陷**：Linux 候选名原先只有 `libopenvino_c.so`，而 pip 安装的 OpenVINO 只提供 `libopenvino_c.so.2640`（SONAME 也是它，没有无版本号的符号链接，因为 wheel 没有理由携带只有链接器才用的符号链接）。于是本包在一个完整可用的安装上直接 `OpenVinoLibraryError`。先测后改：默认候选下失败并留下完整诊断，把名字覆盖为全路径后 9 个 smoke 测试全过，从而确定唯一的问题就是名字。修复为两个候选——先 `libopenvino_c.so`（archive/apt 布局有该符号链接，也是上游文档里的名字），再 `libopenvino_c.so.<后缀>`；后缀由 `version.nim` 的三个数字推导（`2026.4.0` → `2640`），不再手写第二份版本号。新增 `tests/unit/tlibrary.nim` 13 项测试同时钉住推导规则与实测到的字面量，并按平台分支断言候选顺序。修复后 Linux 侧 `nimble testSmoke` 无需任何覆盖即通过。
...
- [x] C14 创建 C ABI probe，覆盖 size/align/offset/enum/constants。证据：`tests/abi/abi_probe.c` 对着固定 header 编译并链接进测试；21 个 ABI 测试覆盖 status 与 element type 全部取值、关键 struct 的 sizeof 与 offsetof、bool 与 size_t 宽度、以及嵌套 profiling enum。（此处曾记为 22，是计数错误；Linux 实测与文件逐一清点都是 21：scalar 5、status 4、element type 4、struct layout 7、profiling enum 1。）
...
- [x] C18 Windows/Linux 2026.4.0 ABI 与 raw smoke 全部通过。证据：两个平台 `nimble testAbi` 各 21 项、`nimble testSmoke` 各 9 项全部通过。两台机器装的是同一个上游构建 `2026.4.0-22959-99c81491cc3-releases/2026/4`，因此这是同一 ABI 在两套编译器与两套 C 库上的对照，而不是两个不同版本的巧合一致。Linux 侧首次运行暴露了 C12 的一个真实缺陷，见下。
...
- [x] F04 ABI 测试覆盖全部 0.1.0 关键类型、常量和符号。证据：`nimble testAbi` 21 个测试由 C 探针驱动，header 中存在而绑定缺失会让计数检查失败；Windows 与 Linux 各跑一遍，21/21 通过，且两台机器装的是同一个上游构建 `2026.4.0-22959-99c81491cc3`。`nimble testSmoke` 对真实 runtime 解析 55 个必需符号。反向测试过：把 `U8` 改回原型的 13 会让 2 个测试失败。
- [x] F05 Windows/Linux 完整 CPU 推理得到固定输出。证据：两个平台各自 `nimble testIntegration` exit 0，debug 与 release 各 50 个测试，共 100 个 `[OK]`、0 个 `[FAILED]`，ReLU 输出都是 `@[0.0, 2.0, 0.0, 4.0]`。Linux 侧为 Ubuntu 26.04.1 LTS、内核 7.0.0-34、Nim 2.2.4、OpenVINO 2026.4.0（pip 安装）。注意 Linux 用的 Nim 是 2.2.4 而 Windows 是 2.2.12，两个版本都实测过，这比只测一个版本更有说服力。
...
- [x] H01 所有 Phase Gate 都有可复核证据。证据：A–G 各 Checklist 项均给出文件、命令或测试依据；公开仓库 [CI #36056011323](https://github.com/AbyssGG/OpenVINO-Nim-API/actions/runs/36056011323) 验证 Node.js 24 action 下的完整 push 矩阵，[release dry run #36056222987](https://github.com/AbyssGG/OpenVINO-Nim-API/actions/runs/36056222987) 验证归档链路而未发布。
